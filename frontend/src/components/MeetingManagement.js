...
function MeetingManagement() {
  ...
  const handleUpdateStatus = async (meetingId, newStatus) => {
    try {
      ...
      toast.error(error.response?.data?.detail || 'Failed to update meeting status');
    }
  };
  ...
  const openEditDialog = (meeting) => {
    ...
    const duration = Math.round((new Date(meeting.end_time) - startTime) / (1000 * 60));
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'rescheduled': return <CalendarIcon className="h-4 w-4 text-blue-500" />;
        ...
        <form onSubmit={handleCreateMeeting} className="space-y-4">
          <div>
            ...
            <Select
              ...
              onValueChange={(value) => setNewMeeting({...newMeeting, lead_id: value})}
            >
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(value) => setNewMeeting({...newMeeting, duration_minutes: parseInt(value)})}
            >
            ...
            <div>
              <div className="p-4 bg-red-50 rounded-lg">
                <h4 className="font-medium text-red-900">
                  {selectedMeeting.title || `Meeting with ${getLeadName(selectedMeeting.lead_id)}`}
                </h4>
      ...
      {meetings.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {meetings.map((meeting) => (
            <Card key={meeting.id} className="card-hover" data-testid={`meeting-card-${meeting.id}`}>
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <CardTitle className="text-lg truncate">
                      {meeting.title || `Meeting with ${getLeadName(meeting.lead_id)}`}
                    </CardTitle>
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <p className="text-gray-500 mb-6 max-w-md mx-auto">
              Schedule your first meeting with a lead to start building relationships and closing deals.
            </p>
            <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
              <DialogTrigger asChild>
